...
// Récupérer les sacs avec image et logo
$sql_sacs = "SELECT *, image as image_sac, logo as logo_marque FROM sac";
$result_sacs = $connecte->query($sql_sacs);
$sacs = [];
if ($result_sacs->num_rows > 0) {
    while ($row = $result_sacs->fetch_assoc()) {
        $sacs[] = $row;
    }
}
...
<script>
  ...
  gtag('config', 'G-WM0VMRQ8D0');
    ...
    <title>Sacs - Kama Boutique</title>
...
<body>
    ...
    <div class="loading-overlay" id="loadingOverlay">
        ...
        <div class="loading-text">Chargement des sacs...</div>
    </div>
    ...
    <header>
        ...
        <nav class="nav">
            ...
            <a href="chaussure.php">
                <button class="nav-button">
                    <i class="fas fa-shoe-prints" id="chaussure"></i>
                    <span class="sr-only">Chaussure</span>
                </button>
            </a>
            <a href="sac.php">
                <button class="nav-button">
                    <i class="fas fa-shopping-bag" style="color:#2563eb;" id="sac"></i>
    ...
    <div class="container">
        <div class="product-grid">
            <?php if (empty($sacs)): ?>
                <div class="no-sacs-container">
                    <img src="aucun.webp" alt="Pas de sacs disponibles" class="no-sacs-image">
                ...
                <?php foreach ($sacs as $sac): ?>
                    <div class="product-item">
                        <div class="product-header">
                            <?php if (!empty($sac['logo_marque'])): ?>
                                <img src="<?= $sac['logo_marque'] ?>" 
                                     alt="Logo <?= htmlspecialchars($sac['marque']) ?>" 
                                     class="brand-logo">
                            <?php else: ?>
                                <div class="brand-logo" style="display: flex; align-items: center; justify-content: center;">
                                    <i class="fas fa-shopping-bag" style="color: var(--primary-color); font-size: 1.5rem;"></i>
                                </div>
                            <?php endif; ?>
                            <div class="product-title">
                                <h3><?= htmlspecialchars($sac['marque']) ?></h3>
                            </div>
                        </div>
                        
                        <?php if (!empty($sac['image_sac'])): ?>
                            <img src="<?= $sac['image_sac'] ?>" 
                                 alt="Sac <?= htmlspecialchars($sac['marque']) ?>" 
                                 class="product-image" 
                        ...
                        <div class="product-details">
                            <p><strong>Couleur:</strong> <?= htmlspecialchars($sac['couleur']) ?></p>
                            <p><strong>Prix:</strong> <?= htmlspecialchars($sac['prix']) ?> $</p>
                        </div>
                        
                        <a href="https://wa.me/<?= $whatsapp_number ?>?text=Bonjour, je suis intéressé(e) par le sac de marque <?= urlencode(htmlspecialchars($sac['marque'])) ?> de couleur <?= urlencode(htmlspecialchars($sac['couleur'])) ?> au prix de <?= urlencode(htmlspecialchars($sac['prix'])) ?> $." 
    ...
    <footer class="footer">
        ...
        <div class="footer-copyright">
            &copy; <?php echo date("Y"); ?> Novilink. Tous droits réservés.
        </div>
        <br><br>
    </footer>
